...
REQ_HASH="$(compute_sha256 src/requirements.txt)-$(compute_sha256 src/requirements-linux.txt)"
OLD_REQ_HASH=""
if [ -f "$REQUIREMENTS_HASH_FILE" ]; then
    OLD_REQ_HASH="$(cat "$REQUIREMENTS_HASH_FILE" || true)"
fi

if [ "$VENV_NEEDS_RECREATION" = true ] || [ "$REQ_HASH" != "$OLD_REQ_HASH" ]; then
    echo "Installing/updating dependencies..."
    pip install --upgrade pip
    pip install -r src/requirements.txt -r src/requirements-linux.txt
    echo "$REQ_HASH" > "$REQUIREMENTS_HASH_FILE"
else
    echo "Dependencies are up to date (skipping pip install)"
fi

echo "Starting Snipping Lens (close with Ctrl+C)..."
python src/main.py
